...
component extends="org.lucee.cfml.test.LuceeTestCase" labels="mcpserver" {
    ...
    function beforeAll() {     
        variables.tool=new org.lucee.extension.mcp.tools.Tags()  
    }
    ...
    function run(testResults, testBox) {
        ...
        // -------------------------------------------------------------------------
        describe("checking the Tags tool", function() {

            it("checking name", function() {
                expect(	tool.getName() ).toBe( "get_lucee_tag" );
            }); 
            ...
            it("checking exec", function() {
                var result=tool.exec({name:"loop"});
